...
class ProphetForecaster(BaseForecaster):
    ...
    def _make_prophet(self):
        ...
        us_holidays = pd.DataFrame(
            [{"holiday": name, "ds": pd.Timestamp(date)} for date, name in hol.US(years=range(2015, 2030)).items()]
        )
    ...
    def fit(self, train_data: pd.DataFrame, target_col: str = "total") -> None:
        df = train_data.reset_index() if isinstance(train_data.index, pd.DatetimeIndex) else train_data.copy()
    ...
    def predict(self, horizon: int) -> pd.DataFrame:
        ...
        future = self.model.make_future_dataframe(periods=horizon, freq="W-MON", include_history=False)
