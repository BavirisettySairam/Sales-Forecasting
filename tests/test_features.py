...
from src.features.engineering import create_features, drop_warmup_rows, get_feature_columns
...
def weekly_df():
    ...
    for state in ["California", "Texas"]:
        ...
        for date in dates:
            rows.append({"state": state, "date": date, "total": max(0.0, base + rng.normal(0, 5000)), "category": "Beverages"})
...
def test_get_feature_columns_all_numeric(weekly_df, config):
    ...
    for col in feat_cols:
        assert result[col].dtype in [np.float64, np.int64, np.int32, np.float32, np.uint32]
...
def test_lag_features_do_not_leak_across_states(weekly_df, config):
    ...
    for state in ["California", "Texas"]:
        state_df = result[result["state"] == state].sort_values("date").reset_index(drop=True)
        assert np.isnan(state_df.loc[0, "lag_1"]), f"lag_1 at row 0 for {state} should be NaN"
...
def test_rolling_mean_uses_past_data_only(weekly_df, config):
    result = create_features(weekly_df, config)
    ca = result[result["state"] == "California"].sort_values("date").reset_index(drop=True)
    # rolling_mean_4 at row i = mean of total[max(0,i-4) : i] (shift(1) means window ends at i-1)
    for i in range(5, 15):
...
def test_rolling_feature_at_t_does_not_use_total_at_t(weekly_df, config):
    ...
    assert feat_mod_ca.loc[10, "rolling_mean_4"] == pytest.approx(original_rolling, rel=1e-4)
