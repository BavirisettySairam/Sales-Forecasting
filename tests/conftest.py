...
def mock_redis():
    ...
    redis.incr.side_effect = lambda k: store.update({k: store.get(k, 0) + 1}) or store[k]
...
def base_config():
    return {
        ...
        "holiday_country": "US",
        "sarima": {"seasonal_period": 4, "stepwise": True, "max_p": 1, "max_q": 1,
                   "max_P": 1, "max_Q": 1, "D": 1, "alpha": 0.05},
        "prophet": {"interval_width": 0.95, "yearly_seasonality": False,
